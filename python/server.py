...
def api_report():
    ...
    for a in apps:
        app_name = (a.get("name") or "").strip().lower()
        version = (a.get("version") or "").strip()
        install_date = (a.get("install_date") or "").strip()
        ...
        if not last or (last[0] or "").strip() != version or int(last[1] or 0) != size_bytes:
            cur.execute("""
